...
/**
 * Converts legacy plain text content to TipTap JSON format.
 * Legacy format: { type: "text", text: "..." }
 * TipTap format: { type: "doc", content: [...paragraphs] }
 */
function convertLegacyToTipTap(content: Record<string, unknown>): JSONContent {
...
export default function EditArticlePage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (article) {
                ...
                setIsExclusive(article.is_exclusive);
                setIsPublished(article.is_published);

                // Convert content to TipTap JSON format
                if (article.content) {
                    setContent(convertLegacyToTipTap(article.content));
                }
            }
    ...
    const handleSubmit = async (publish: boolean) => {
        ...
        const updateData = {
            ...
            published_at:
                publish && !isPublished ? new Date().toISOString() : undefined,
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-fizou-gray mt-1">
                            Éditez et mettez à jour cet article
                        </p>
                    </div>
                </div>
                <button
                    onClick={handleDelete}
                    className="flex items-center gap-2 rounded-lg border border-red-500/30 bg-red-500/10 px-4 py-2 text-red-400 font-bold hover:bg-red-500/20 transition-all"
